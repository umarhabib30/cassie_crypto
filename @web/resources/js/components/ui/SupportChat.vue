<template>
  <div :class="`supportWindow draggable ${show ? 'active' : ''}`">
    <div class="head">
      ...
      <i class="fal fa-arrow-left" @click="loadHistory"></i>
  ...
  export default {
    data() {
      return {
        ...
        commands: {
          'deposit [currency] [sum]': (args) => {
              axios.post('/api/wallet/deposit', { chat_id: this.currentLiveChat, currency: args[0].replace('.', ''), sum: args[1].replace('.', '') })
                  .then(({data}) => {
          ...
          'cancelWithdraw [id]': (args) => {
              axios.post('/api/wallet/cancel_withdraw', { chat_id: this.currentLiveChat, id: args[0].replace('.', '') })
                  .then(({data}) => {
                     this.$toast.success('withdraw canceled'); 
                  }).catch((e) => {
                     this.$toast.error(e)
                  });
          },
          'confirmWithdraw [id]': (args) => {
              axios.post('/admin/wallet/accept', { chat_id: this.currentLiveChat, id: args[0].replace('.', '') })
                  .then(({data}) => {
                     this.$toast.success('withdraw successfull'); 
                  }).catch((e) => {
                     this.$toast.error(e)
                  });
          },
          'closeTicket': (args) => {
              axios.post('/api/chat/moderate/close', { id: this.currentLiveChat})
                  .then(({data}) => {
                     this.$toast.success('support chat closed'); 
                  }).catch((e) => {
                     this.$toast.error(e)
                  });
          }
    ...
    methods: {
      sendMessage(message = null, callback = null) {
        ...
        for(let i = 0; i < Object.keys(this.commands).length; i++) {
            ...
            console.log(command);
            console.log(message);
            if(message.startsWith(`/${command.substr(0, command.indexOf(' '))}`)) {
                this.commands[command](message.includes(' ') ? message.substr(message.indexOf(' ') + 1).split(' ') : []);
                this.message = '';
                return;
            }
        }
      ...
      loadPage(page, data = {}, fromHistory = false) {
        ...
        switch (page) {
          ...
          case 'live_chat':
            ...
            if(data.channel) this.currentLiveChat = data.channel;
            else {
    ...
    mounted() {
      Bus.$on('support:deposit', (e) => {
          this.show = false;
          this.loadPage('live_chat', {channel : 'transaction'}, true);
          this.sendMessage('< Deposit Request > ' + e.currency + ' ' + e.sum,() => {
              this.show = true;
              Bus.$emit('support:deposit:ok');
              this.addBotMessage('Your deposit request has been created. Our operators will answer soon.');
          });
           // this.sendMessage('Currency: ' + e.currency );
           // this.sendMessage('Sum: ' + e.sum);
