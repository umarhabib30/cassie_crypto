<template>
  <div v-if="liveGames.length > 0" class="container pageContent" id="live-games">
    <div class="game-list">
      <div class="category">
        <div class="icon">
...
export default {
  ...
  methods: {
    ...
    loadInitialGames() {
      this.visibleGames = this.liveGames.slice(0, this.gamesToLoad);
    },
    loadMoreGames() {
      ...
      setTimeout(() => {
        ...
        this.visibleGames = this.visibleGames.concat(this.liveGames.slice(start, end));
        this.isLoadingMore = false;
      }, 500);
    },
    onScroll() {
      const bottomOfWindow = window.innerHeight + window.scrollY >= document.documentElement.offsetHeight - 200;
      if (bottomOfWindow && this.visibleGames.length < this.liveGames.length) {
        this.loadMoreGames();
      }
    }
  }
