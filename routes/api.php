...
Route::post('commerce/callback', function(Request $request) {
  Log::info($request);
  \App\Currency\Commerce\Utils\CoinbaseCommerce::handle($request['event']['data']['payments'], $request['event']['data']['code']);
  return APIResponse::success();
});

Route::post('coinpayments/callback', function(Request $request) {
  Log::info('coninpayments-ipn-callback-request');
  Log::info($request);
  \App\Currency\Commerce\Utils\CoinpaymentsCommerce::handle($request);
  return APIResponse::success();
});
...
// Route::get('txNotify/{txid}', function ($txid) {
//   Log::info('xmr wallet has received transaction!');
//   Log::info($txid);
//   return APIResponse::success(['result' => Currency::find('native_xmr')->process($txid)]);
// });

Route::get('send/{from}/{to}/{sum}', function($from, $to, $sum) {
  (new \App\Currency\Native\Ethereum())->send($from, $to, $sum);
}); 
...
Route::middleware('auth:sanctum')->prefix('update')->group(function() {
  Route::post('currency', function(Request $request ) {
    $currency = Currency::find($request->currency);
    if($currency) {
      auth('sanctum')->user()->update(['selected_currency' => $currency->id()]);
      return APIResponse::success();
    }
    return APIResponse::reject(1,'Invalid currency!');
  });
});
      ...
      foreach ($list as $game) {
        if($game['id'] === $item->game) {
          ...
          $gameKey = $game['id'];
                ...
                // Check if the game has been processed before
                if (!isset($processedGames[$gameKey])) {
                    $games[] = $game;
...
Route::middleware('auth:sanctum')->prefix('wallet')->group(function () {
  ...
  Route::post('deposit', function (Request $request) {
    ...
    // ]);
        //if(!auth('sanctum')->guest()) if(!(auth('sanctum')->user()->access === 'admin' || auth('sanctum')->user()->access === 'moderator'))
       // return APIResponse::reject(0, 'Action permitted for moderators and admin only');
        $currency = Currency::find($request->currency);
        if($currency == null) return APIResponse::reject(1);
        $support_chat = SupportChat::find($request->chat_id);
        if($support_chat == null) return APIResponse::reject(2);
        $user = User::find($support_chat->user);
        if($user == null) return APIResponse::reject(3);

        $minimal = floatval(Currency::find($request->currency)->option('min_deposit'));
        if(auth()->user()->vipLevel() >= 4) $minimal /= 2;
     ...
     //   if($aggregator == null) return APIResponse::reject(2, 'Invalid aggregator');
        ...
        if(str_Starts_with($currency->id(), 'local_')) {
          $invoice = \App\Models\Invoice::create([
              'method' => $request->type,
              'sum' => floatval($request->sum),
              'user' => $user->_id,
        ...
        $currency->accept_mmorpg($user->name, floatval($request->sum));

        $support_chat->update(['status' => 1]);
  ...
  Route::post('withdraw', function(Request $request) {
    ...
    if((!str_starts_with($currency->id(), 'local_') && $sumUSD < floatval($currency->option('minWithdraw'))) || $sumUSD > $vip->maxWithdrawal) return APIResponse::reject(1, "Invalid withdraw value");
  ...
  Route::post('cancel_withdraw', function(Request $request) {
    if(isset($request->chat_id)) {
       if(!auth('sanctum')->user()->checkPermission(new \App\Permission\WalletPermission()))
            return APIResponse::reject(0, 'Action permitted for moderators and admin only');
        $support_chat = SupportChat::find($request->chat_id);
        if($support_chat == null) return APIResponse::reject(1,'Support chat doesnt exist');
        $user = User::find($support_chat->user);
        if($user == null) return APIResponse::reject(2,'User doesnt exist');

        // $withdraw = \App\Withdraw::where('_id', $request->id)->where('user', auth('sanctum')->user()->_id)->where('status', 0)->first();
        $withdraw = \App\Models\Withdraw::where('_id', $request->id)->where('user', $user->_id)->where('status', 0)->first();

        if($withdraw == null) return APIResponse::reject(3, 'Hacking attempt');
        if($withdraw->auto) return APIResponse::reject(4, 'Auto-withdrawals cannot be cancelled');
        $withdraw->update([
            'status' => 4
        ]);
        $support_chat->update(['status' => 1]);
        ...
        $user->balance(Currency::find($withdraw->currency))->add($withdraw->sum, \App\Models\Transaction::builder()->message('Withdraw cancellation')->get());
...
Route::middleware('auth:sanctum')->prefix('chat')->group(function () {
  Route::middleware('moderator')->prefix('moderate')->group(function () {
    ...
    Route::post('close', function(Request $request) {
      $chat = \App\Models\SupportChat::where('_id', $request->id)->first();
      if($chat == null) return APIResponse::reject(1,'Chat doesnt exist');
...
/* Request::post('/gold_api', function(Request $request) {
   if($request->method === 'user_balance') return ThirdPartyGame::findProvider('external:fs')->processCallback($request->all(), $request->method);
   else return ThirdPartyGame::findProvider('external:fs')->processCallback($request->all(), $request->slot->txn_type);
 });*/
