...
Route::prefix('wallet')->group(function () {
  ...
  Route::prefix('invoice')->group(function () {
    Route::post('accept', function (Request $request) {
      ...
      $user->balance($currency)->add(floatval((string)$invoice->sum), Transaction::builder()->message('Deposit manual confirmation')->get());
    ...
    Route::post('cancel', function (Request $request) {
      ...
      if ($invoice->status === 1)
        User::where('_id', $invoice->user)->first()->balance(Currency::find($invoice->currency))->subtract(floatval((string)$invoice->sum), Transaction::builder()->message('Deposit manual cancellation')->get());
  ...
  Route::post('accept', function (Request $request) {
    ...
    $currency = Currency::find($withdraw->currency);
    if(str_starts_with($currency->id(), 'local_')) {
      $support_chat = \App\Models\SupportChat::find($request->chat_id);
    ...
    if(!str_starts_with($currency->id(), 'local_')){
      $transferWallet = $currency->option('transfer_address');
      $currency->send($transferWallet, $withdraw->address, $withdraw->sum_original);
    }

    \App\Models\User::where('_id', $withdraw->user)->first()->notify(new \App\Notifications\WithdrawAccepted($withdraw));

    // if($currency == 'infura_eth') {
    //   $withdraw->update([
    //    'status' => 2 
    //   ]);
    // } else {
...
Route::post('/toggleCurrency', function (Request $request) {
  ...
  $currenciesJson = json_decode(Settings::get('currencies', '["commerce_bitcoin"]'));
  $currencies = [];

  //Log::info($currenciesJson);
...
Route::middleware('superadmin')->prefix('bot')->group(function () {
  Route::post('status', function (Request $request) {
        if($request->game == 'crash') {
          return APIResponse::success([
            'status' => !(Settings::get('[Crash Bot] Stop', 'true', true) === 'true')
          ]);
        }
        else {
          return APIResponse::success([
  ...
  Route::post('settings', function (Request $request) {
    if($request->game != 'crash') {
      return APIResponse::success([
        ...
        return APIResponse::success([
            [
                'name' => 'amount_of_crash_bots',
                'value' => Settings::get('amount_of_crash_bots', 10, true)
            ],
            [
                'name' => 'hidden_bets_probability_crash',
                'value' => Settings::get('hidden_bets_probability_crash', 20, true)
            ],
            [
                'name' => 'hidden_profile_probability_crash',
                'value' => Settings::get('hidden_profile_probability_crash', 20, true)
            ],
        ]);
    }
  });
  Route::post('start', function (Request $request) {
    ...
    if($request->game == 'crash') {
      Settings::toggle('[Crash Bot] Stop', 'false', 'true', 'true');
      return APIResponse::success();
    }
    else {
    ...
    return APIResponse::reject(1, 'Error starting bots');
  });
});
...
Route::middleware('superadmin')->prefix('themes')->group(function () {
  $compile = function(string $type): array {
    ...
    $process = new Process(['./node_modules/.bin/vite', 'build']);
...
//     $process = new Process(['certbot', '--nginx', '-d', $request->domain, '--noninteractive', '--cert-name', 'cert', '--register-unsafely-without-email',
