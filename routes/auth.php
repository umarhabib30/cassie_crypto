...
if (!function_exists('createUser')) {
  function createUser($email, $login, $password, $avatar = null, $additionalData = []): User {
    $user = User::create(array_merge([
      ...
      'confirmation_token' => ProvablyFair::generateServerSeed(),
...
Route::post('resetPassword', function (Request $request) {
  ...
  $user = User::where('email', $request->login)->orWhere('name', $request->login)->where('email_verified_at',"!=", null)->first();
  if (!$user) return APIResponse::reject(4, 'Invalid user');
...
Route::post('confirmEmail', function(Request $request) {
    if($request->token) {
        $user = User::where('confirmation_token', $request->token)->first();
        if(!$user) return APIResponse::reject(1, 'Invalid token');
        $user2 = User::where('email', $user->email)->where('email_verified_at', '!=', null)->first();
        if($user2) return APIResponse::reject(2, 'Email already in use');
        $user->update(['confirmation_token' => null, 'email_verified_at' => Carbon::now() ]);
        return APIResponse::success();
    }
...
Route::post('/register', function (Request $request) {
  $request->validate([
    ...
    'name' => ['required', 'unique:users', 'string', 'max:24', 'regex:/^[a-zA-Z0-9]{4,64}$/u'],
    'password' => ['required', 'string', 'min:5']
    //'captcha' => ['required']
  ]);
  ...
  //Mail::to($user)->send(new ConfirmEmail($user->confirmation_token));
