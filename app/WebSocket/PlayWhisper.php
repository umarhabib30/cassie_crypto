...
class PlayWhisper extends WebSocketWhisper {
  ...
  public function process($data): array {
    ...
    if($data->fiat) $data->bet = $c->convertUSDToToken($c->fiatNumberFormat(floatval($data->bet)));
...
/*    try {
    ...
    $data = new Data($this->user, [
      ...
      'autoCashoutMultiplier' => $game->metadata()->id() == 'crash' ? ($data->autoCashout ?? 0)  : null
    ]);

    // TODO push this to laravel job, no need to reward referrals immediately
    ...
    // TODO push this to laravel job, no need to increase game counter before return game result to player
