...
abstract class Game {
  ...
  public function nonce($id = null) {
    if($id) return \App\Models\Game::where('id', $id)->first()->nonce;
    return auth()->guest() ? mt_rand(1, 100000) : \App\Models\Game::where('user', auth()->user()->_id)->count() + 1;
  }

  public function server_seed($id = null) {
    if($id) return \App\Models\Game::where('id', $id)->first()->server_seed;
    return ProvablyFair::generateServerSeed();
  }

  public function client_seed($id = null) {
    if($id) return \App\Models\Game::where('id', $id)->first()->client_seed;
