...
abstract class Currency {
  ...
  public function tokenPrice($fiat = 'usd'): float {
    try {
      if (!Cache::has('conversions:' . $this->alias()))
        if(str_starts_with($this->id(), 'local_')) {
          if($this->id() == 'local_osrs') Cache::put('conversions:' . $this->alias(), 0.22, now()->addHours(1));
          else if($this->id() == 'local_rs3') Cache::put('conversions:' . $this->alias(), 0.04, now()->addHours(1));
        }
        else {
      ...
      if(str_starts_with($this->id(), 'local_')) return Cache::get('conversions:' . $this->alias());
  ...
  public function convertTokenToFiat(float $tokenAmount, string $fiat = 'eur') {
    return $tokenAmount * $this->tokenPrice($fiat);
  }

  public function convertFiatToToken(float $fiatAmount, string $fiat = 'eur') {
  ...
  public static function getAllSupportedCoins(): array {
    ...
    //  new Token\TokenUSDC(),
     // new Token\TokenUSDT(),
      ...
      new Local\OSRS(),
      new Local\RS3(),
      new Native\TokenUSDC(),
      new Native\TokenUSDT(),
      new Native\TokenLink(),
      new Native\Monero(),
      new Native\Bitcoin(),
      new Native\Litecoin(),
      new Native\Dogecoin(),
  ...
  public function acceptThirdParty(int $confirmations, User $user, string $id, float $sum, int $requiredConfirmations): bool {
    ...
    if ($invoice->status == 0 && $invoice->confirmations >= 1) {
  ...
  public function accept(int $confirmations, string $wallet, string $id, float $sum): bool {
    ...
    if ($invoice->status == 0 && $invoice->confirmations >= intval($this->option('confirmations'))) {
      ...
      if($this->id() != 'native_xmr' && $this->id() != 'token_usdc') {
        $transferWallet = $this->option('transfer_address');
        $this->send($wallet, $transferWallet, $sum);
      }

      Log::info('Paying out referals deposit bonuses!');
  ...
  public function accept_mmorpg(string $name, float $sum) {
